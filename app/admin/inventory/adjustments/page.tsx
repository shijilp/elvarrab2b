...
type Line = {
  productInput: string; // ID or SKU
  product?: ProductMini; // resolved
  quantity: number | ""; // +increase / -decrease
  batch_no?: string;
};
...
export default function NewAdjustmentPage() {
  ...
  const [lines, setLines] = useState<Line[]>([
    { productInput: "", quantity: "", batch_no: "" },
  ]);
  ...
  const addLine = () =>
    setLines((p) => [...p, { productInput: "", quantity: "", batch_no: "" }]);
  ...
  const submit = async () => {
    setErr(null);
    setOk(null);
    if (!warehouse) return setErr("Select a warehouse.");
    const prepared = [];
    ...
    <div className="mx-auto max-w-6xl p-6 space-y-6 text-neutral-200">
      <nav className=" max-w-7xl px-4 sm:px-6 lg:px-1 py-2  text-xs opacity-80">
        <ol className="flex items-center gap-2">
          ...
          <li>›</li>
          <li className="opacity-90">{"Adjustments"} </li>
        </ol>
      </nav>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">
          <span className="text-[#d4af37]">Elvarra</span> · New Adjustment
        </h1>
      </div>
      ...
      <div className={cx(card, "p-4 grid grid-cols-1 md:grid-cols-2 gap-4")}>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-sm text-neutral-400">Reason (optional)</span>
          <input
            className={input}
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            placeholder="Stock count / Damage / Rework"
          />
        </label>
      </div>
