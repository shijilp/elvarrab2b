...
export const api2 = axios.create({
  baseURL: "/api/proxy/proxy", // do NOT expose backend URL
...
api2.interceptors.request.use((config) => config);
  ...
  async (err: AxiosError) => {
    ...
    if (isRefreshing) {
      return new Promise((resolve) => {
        queue.push(() => resolve(api2.request(original)));
      });
    }
